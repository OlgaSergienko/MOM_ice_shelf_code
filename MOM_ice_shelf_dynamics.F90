...
use MOM_ice_shelf_initialize, only : initialize_ice_flow_from_file
use MOM_ice_shelf_initialize, only : initialize_ice_shelf_boundary_channel
use MOM_ice_shelf_initialize, only : initialize_ice_thickness_channel
...
type, public :: ice_shelf_dyn_CS ; private
  ...
  real :: input_flux ! < ice flux at the inflow boundary [Z^2 T-1 ~> m2 s-1]
  real :: input_thickness ! < ice thickness at the inflow boundary [Z ~> m]
...
# include "version_variable.h"
  ...
  if (active_shelf_dynamics) then
    ...
    call get_param(param_file, mdl, "A_GLEN_ISOTHERM", CS%A_glen_isothermal, &
                 ...
                 units="Pa-3 s-1", default=2.2261e-25, scale=1.0) !OVS change units to Pa-3 s-1
!                 units="Pa-3 yr-1", default=9.461e-18, scale=1.0/(365.0*86400.0))
    ...
    call get_param(param_file, mdl, "MIN_STRAIN_RATE_GLEN", CS%eps_glen_min, &
                 ...
                 units="s-1", default=1.e-19, scale=US%T_to_s) !OVS change units to s-1
    ...
    call get_param(param_file, mdl, "BASAL_FRICTION_COEFF", CS%C_basal_friction, &
                 ...
                 units="Pa (m s-1)^(n_basal_fric)", scale=US%kg_m2s_to_RZ_T**CS%n_basal_fric, &  ! OVS change units to s-1
    ...
    call get_param(param_file, mdl, "INPUT_FLUX_ICE_SHELF", CS%input_flux, &
            "volume flux at upstream boundary", &
                 units="m2 s-1", default=0., scale=US%T_to_s/(365.0*86400.0))
  ...
  if (active_shelf_dynamics .and. .not.new_sim) then
    ...
    call pass_vector(CS%u_shelf, CS%v_shelf, G%domain, TO_ALL, BGRID_NE) !OVS 
  endif
...
!    call initialize_ice_thickness_channel(ISS%h_shelf,ISS%area_shelf_h, ISS%hmask, G, US, param_file)
...
subroutine ice_shelf_solve_outer(CS, ISS, G, US, u_shlf, v_shlf, iters, time)
 ...
 ! call initialize_ice_flow_shelf(CS, ISS, G, US,ISS%h_shelf, ISS%area_shelf_h, ISS%hmask, u_shlf,v_shlf) 
...
!     if (ISS%h_shelf(i,j) < rhow/rho * G%bathyT(i,j)) then
  ...
  do j=jsc-1,jec+1
    do i=isc-1,iec+1
!    do i=isc-G%domain%nihalo+1,iec+G%domain%nihalo-1
      cnt = 0
      sx = 0
      sy = 0
      dxh = G%dxT(i,j)
...
! returns the diagonal entries of the matrix for a Jacobi preconditioning
  ...
  do j=jsc-1,jec+1  
    ...
    do iq=1,2 ; do jq=1,2 ; do iphi=1,2 ; do jphi=1,2 ; Itgt = I-2+iphi ; Jtgt = J-2+jphi !Jtgt = J-2-jphi !OVS fix index
      ilq = 1 ; if (iq == iphi) ilq = 2
      jlq = 1 ; if (jq == jphi) jlq = 2
...
! (and possibly other variables) will be updated in this function
  ...
  do j=jsc-1,jec+1 
     do i=isc-1,iec+1 ; if (ISS%hmask(i,j) == 1) then
...
subroutine initialize_ice_flow_shelf(CS, ISS, G, US,h_shelf, area_shelf_h, hmask, u_shlf,v_shlf)
  ...
  real, dimension(SZDI_(G),SZDJ_(G)), &
                         intent(in) :: area_shelf_h !< The area per cell covered by the ice shelf [L2 ~> m2].
  real, dimension(SZDI_(G),SZDJ_(G)), &
                         intent(in) :: hmask !< A mask indicating which tracer points are
                                             !! partly or fully covered by an ice-shelf
  real, dimension(SZDIB_(G),SZDJB_(G)), &
  ...
  real, dimension(SZDIB_(G),SZDJB_(G)) :: taudx, taudy, h_all ! Driving stresses at q-points [R L3 Z T-2 ~> kg m s-2]                
  character(len=40)  :: mdl = "initialize_ice_shelf_stream_velocity1D" ! This subroutine's name.
  real :: Visc_coef, eps_glen_min, C, Bn, Qgl, Hgl, adot, n_gln
  real :: edge_pos, shelf_slope_scale, rhoi, rhoi_rhow, denom
  integer :: i, j, jsc, jec, jsd, jed, jedg, nyh, isc, iec, isd, ied, n_glen,n_basal_fric
  integer :: j_off
  ...
  taudx(:,:) = 0.0 ; taudy(:,:) = 0.0 ; h_all(:,:) = 0.0

  call calc_shelf_driving_stress(CS, ISS, G, US, taudx, taudy, CS%OD_av)

  adot =0.3/(86400*365.25)  ! accumulation rate to compute ice shelf thickness
  n_gln = CS%n_glen
  rhoi = CS%density_ice
  rhoi_rhow = CS%density_ice / CS%density_ocean_avg
  Qgl = CS%input_flux
  Hgl = CS%input_thickness
  C   = CS%C_basal_friction

  Visc_coef = 1.68e8 !US%kg_m2s_to_RZ_T*US%m_to_L*US%Z_to_L*(CS%A_glen_isothermal)**(-1./CS%n_glen)
  Bn=(rhoi*CS%g_Earth*(1.-rhoi_rhow)/8/Visc_coef)**n_gln
  denom = 0.0   
...
!        do i=isc-1,iec+1
    do j=jsc,jec
        do i=isc,iec
              denom = Qgl**(n_gln+1.)*(adot - Bn*Hgl**(n_gln+1)) + Bn*Hgl**(n_gln+1)*(Qgl +adot*G%geoLonCu(I,J)*G%dxT(I,J))**(n_gln+1.)
              h_all(I,J)=Hgl*(adot/denom*(Qgl + adot*G%geoLonCu(I,J)*G%dxT(I,J))**(n_gln+1) )**(1./(n_gln+1.))
!            if ((rhoi_rhow * h_all(i,j) - G%bathyT(i,j) <= 0) .AND. (hmask(I,J)>0)) then
              !denom = Qgl**(n_gln+1.)*(adot - Bn*Hgl**(n_gln+1)) + Bn*Hgl**(n_gln+1)*(Qgl +adot*G%geoLonCu(I,J)*G%dxT(I,J))**(n_gln+1.)
!              h_shelf(I,J)= h_all(I,J)!Hgl*(adot/denom*(Qgl + adot*G%geoLonCu(I,J)*G%dxT(I,J))**(n_gln+1) )**(1./(n_gln+1.))
!            endif
!            if((CS%ground_frac(i,j)==1) .AND. (hmask(I,J)/= 3) ) then 
            if((rhoi_rhow * h_shelf(i,j) - G%bathyT(i,j) > 0)  .AND. (hmask(I,J)/= 3) ) then
              u_shlf(I,J) = taudx(I,J)/C/area_shelf_h(I,J)
              v_shlf(I,J) = taudy(I,J)/C/area_shelf_h(I,J)
           endif
       enddo
    enddo
!
...
!
!  call get_param(PF, mdl, "A_GLEN_ISOTHERM", A_glen, &
!                 "Ice viscosity parameter in Glen's Law", &
!                 units="Pa-3 yr-1", default=9.461e-18, scale=1.0/(365.0*86400.0))
!                 ! This default is equivalent to 3.0001e-25 Pa-3 s-1, appropriate at about -10 C.
!  call get_param(PF, mdl, "GLEN_EXPONENT", n_glen, &
!                 "nonlinearity exponent in Glen's Law", &
!                  units="none", default=3.)
!  call get_param(PF, mdl, "MIN_STRAIN_RATE_GLEN", eps_glen_min, &
!                 "min. strain rate to avoid infinite Glen's law viscosity", &
!                 units="a-1", default=1.e-12, scale=US%T_to_s/(365.0*86400.0))
!  call get_param(PF, mdl, "BASAL_FRICTION_EXP", n_basal_fric, &
!                 "Exponent in sliding law \tau_b = C u^(n_basal_fric)", &
!                 units="none", fail_if_missing=.true.)
!  call get_param(PF, mdl, "BASAL_FRICTION_COEFF", C_basal_friction, &
!                 "Coefficient in sliding law \tau_b = C u^(n_basal_fric)", &
!                 units="Pa (m yr-1)-(n_basal_fric)", scale=US%kg_m2s_to_RZ_T*((365.0*86400.0)**n_basal_fric), &
!                 fail_if_missing=.true.)
!
!
!  do j=G%jsd,G%jed
...
!  endif ; enddo
!
end subroutine initialize_ice_flow_shelf
